"""
frontend/telas/pg_login.py
Tela de login/cadastro via API FastAPI.
"""
...
def _logo_html() -> str:
    ...
    if os.path.exists(logo_path):
        ...
        return f'<img src="data:image/png;base64,{img_b64}" style="width:80px;display:block;margin-bottom:28px;" />'
    return '<div style="font-size:1.8rem;font-weight:900;color:#00b4a6;margin-bottom:28px;">AprovAI</div>'


def _fazer_login(email: str, senha: str) -> bool:
    """Autentica via API, popula session_state. Retorna True se ok."""
    resp = login(email, senha)
    if not resp:
        return False
    st.session_state.api_token = resp.get("access_token", "")
    perfil = get_me()
    if not perfil:
        return False
    st.session_state.usuario = {
        "id":           perfil.get("id", ""),
        "nome":         perfil.get("nome", resp.get("nome", "")),
        "email":        email,
        "role":         perfil.get("role", "aluno"),
        "horas_por_dia": float(perfil.get("horas_por_dia", 3.0)),
        # Campos complementados no onboarding
        "dias_semana":  5,
        "area_estudo":  "",
        "data_prova":   "",
        "plataformas":  [],
...
[data-testid="stHorizontalBlock"]>[data-testid="stColumn"]:last-child>div{
    padding:0 60px!important;display:flex!important;flex-direction:column!important;
...
@keyframes feat-show {
    0%{opacity:0;transform:translateY(12px)}8%{opacity:1;transform:translateY(0)}
    25%{opacity:1;transform:translateY(0)}33%{opacity:0;transform:translateY(-8px)}
    100%{opacity:0;transform:translateY(-8px)}}
.feat-wrap{position:relative;height:72px;margin-bottom:40px;}
.feat-item{position:absolute;top:0;left:0;display:flex;align-items:flex-start;
    gap:12px;opacity:0;animation:feat-show 12s infinite;}
...
.feat-dot{width:36px;height:36px;border-radius:50%;background:rgba(0,180,166,.18);
    border:1px solid rgba(0,180,166,.35);display:flex;align-items:center;
            ...
            if st.button("Entrar", use_container_width=True, type="primary", key="btn_entrar"):
                if not email_l.strip():
                    ...
                    with st.spinner("Autenticando..."):
                        ok = _fazer_login(email_l.strip(), senha_l)
                    if ok:
                        st.rerun()
                    else:
                        st.error("E-mail ou senha incorretos, ou servidor indisponível.")
            ...
            if st.button("Criar conta", use_container_width=True, type="primary", key="btn_cadastrar"):
                if not nome_c.strip():
                    ...
                    with st.spinner("Criando conta..."):
                        criado = register(nome_c.strip(), email_c.strip(), senha_c)
                    if criado:
                        with st.spinner("Entrando..."):
                            ok = _fazer_login(email_c.strip(), senha_c)
                        if ok:
                            st.rerun()
                        else:
                            st.warning("Conta criada! Faça login para continuar.")
                    else:
                        st.error("Este e-mail já está cadastrado ou servidor indisponível.")
