...
function ModalCriarUsuario({ onClose, onCriado }) {
  const [form, setForm] = useState({ nome: '', email: '', password: '', confirmar_senha: '', role: 'estudante' })
  ...
  async function salvar(e) {
    ...
    setLoading(true)
    try {
      const novo = await criarUsuario({ nome: form.nome, email: form.email, password: form.password, role: form.role })
      onCriado(novo)
      onClose()
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-brand-card border border-brand-border rounded-xl p-6 w-full max-w-md space-y-4 shadow-xl">
        ...
        <form onSubmit={salvar} className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-brand-surface border border-brand-border rounded-lg px-3 py-2 pr-9 text-brand-text text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
              <button
                type="button"
          ...
          <div>
            <label className="text-xs text-brand-muted block mb-1">Confirmar senha</label>
            <div className="relative">
              <input
                required
